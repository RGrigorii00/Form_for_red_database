...
namespace WindowsFormsApp20
{
    public partial class Form4 : Form
    {
        ...
        private void addButton_Click(object sender, EventArgs e)
        {
            ...
            {
                string insertQuery = "INSERT INTO POSTAVSHIK VALUES (@value1, @value2, @value3, @value4)";
                ...
                try
                {
                    using (FbConnection connection = new FbConnection(textBox1.Text))
                    {
                        ...
                        using (FbCommand command = new FbCommand(insertQuery, connection))
                        {
                            ...
                            command.Parameters.AddWithValue("@value1", dataGridView1.CurrentRow.Cells["POSTAVSHIK_ID"].Value);
        ...
        private void deleteButton_Click(object sender, EventArgs e)
        {
            if (dataGridView1.SelectedRows.Count > 0)
            {
                ...
                int selectedId = (int)dataGridView1.SelectedRows[0].Cells["POSTAVSHIK_ID"].Value;

                string deleteQuery = "DELETE FROM POSTAVSHIK WHERE POSTAVSHIK_ID = @id";
                try
                {
                    using (FbConnection connection = new FbConnection(textBox1.Text))
                    {
        ...
        private void updateButton_Click(object sender, EventArgs e)
        {
            ...
            if (dataGridView1.SelectedRows.Count > 0)
            {
                if (!string.IsNullOrEmpty(dataGridView1.CurrentRow.Cells["POSTAVSHIK_ID"].Value.ToString()) && !string.IsNullOrEmpty(dataGridView1.CurrentRow.Cells["FIO"].Value.ToString()) && !string.IsNullOrEmpty(dataGridView1.CurrentRow.Cells["PHONE"].Value.ToString()) && !string.IsNullOrEmpty(dataGridView1.CurrentRow.Cells["ORGANIZATION_ID"].Value.ToString()))
                {
                    string insertQuery = "UPDATE POSTAVSHIK SET POSTAVSHIK_ID = @value1, FIO = @value2, PHONE = @value3, ORGANIZATION_ID = @value4 WHERE POSTAVSHIK_ID = @value1";
